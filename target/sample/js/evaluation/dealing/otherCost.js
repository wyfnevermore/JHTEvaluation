...
function getOtherCost(){
	...
	for(var x=1;x<valueTrs.length-1;x++){
		...
		costResult.push({"costName":costTds[0].children[0].value,"totalCost":costTds[1].children[0].value,"flatOutCount":costTds[2].children[0].value,"unitCost":costTds[3].children[0].value});
	}
	return costResult;
}
...
function viewPage(jsonData){
	...
	//通过表体id把显示文本显示到网页中
	$("#otherCost").append(html);
	$("#otherCostTotal")[0].children[0].children[3].innerText = window.parent.evalHeadData.evalDetail.resultOtherJson;
}
