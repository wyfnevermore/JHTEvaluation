...
function calValFromImpPrice(){
	...
	//1.执行毛利率
	var implGMarginRateVal = (100*(implementPriceVal-prodEvalCostUVal)/prodEvalCostUVal).toFixed(2);
...
function calprodEvalCostU(prodEvalCostVal,specSel){
	var result = 0.00;
	var drawBackRateVal = parseFloat((window.parent.$material_result)[specSel]["drawBackRate"])/100;//退税率
	var transportVal = parseFloat(window.parent.$transport_result);//运杂费
