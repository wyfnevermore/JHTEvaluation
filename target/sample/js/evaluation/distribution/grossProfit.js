...
function changeProfitByProduct(){
	...
	//1.产品成本（USD）
	var prodEvalCostRVal = parseFloat(calEvalCostResultBySpec(curSpec));
 ...
 * 公式：(（产品成本+运杂费+其他费用）/(1-返点率）/（1-第一佣金率）/（1-第二佣金率）)/报价汇率
 * @returns
 */
function calprodEvalCostU(prodEvalCostVal,specSel){
	...
	result = (prodEvalCostVal+otherCostVal)/quotationRateVal;
...
/*
 *计算测算报价
 *公式：（产品成本/（1-客户预算毛利率）+运杂费+其他费用）/(1-返点率）/（1-第一佣金率）/（1-第二佣金率)/报价汇率 
...
function valQuotePrice(prodEvalCostVal,specSel){
	...
	result = (prodEvalCostVal/(1-custGMarginRateVal)+otherCostVal)/quotationRateVal;
	if(typeof(window.parent.$hireCost_result)=="undefined"){
		return result.toFixed(2);
	}
	var hireCost_result = window.parent.$hireCost_result;
...
/*
 * 计算报价金额
 * @param prodEvalCostVal 测算成本
 * @returns
 */
function calQuotePrice(prodEvalCostVal){
	var result = prodEvalCostVal;
	if(typeof(window.parent.$hireCost_result)=="undefined"){
		return result;
	}
	var hireCost_result = window.parent.$hireCost_result;
...
function checkValIsFloat(valueData, selSpec){
	if(typeof(valueData)=="undefined" || !valueData.hasOwnProperty(selSpec) || isNaN(parseFloat(valueData[selSpec]["cost"]))){
		$("#productSize").val(valueData[selSpec]["size"]);
		return 0.00;
	}else{
		$("#productSize").val(valueData[selSpec]["size"]);
		return parseFloat(valueData[selSpec]["cost"]);
	}
}
...
function updateGrossPRow(){
	...
	if(addOrUpdate == "add"){
		var html = "<tr><td>" + $("#spec").find("option:selected").text() + "</td><td>" +$("#productSize").val()
		+ "</td><td>" +$("#custGMarginRate").val()
		+ "</td><td>" +$("#prodEvalCostR").val() + "</td><td>" + $("#prodEvalCostU").val()
		+ "</td><td>" +$("#quotePrice").val() + "</td><td>" +$("#implementPrice").val()
		+ "</td><td>" + $("#implGMarginRate").val() + "</td><td hidden=\"hidden\">" +$("#implGMarginCostU").val()+ "</td><td hidden=\"hidden\">" +$("#implPriceCut").val()
		+ "</td><td><div class=\"btn-group\"><a onclick=\"addGrossProfit()\" href=\"javascript:void(0);\"><i class=\"fa fa-plus\"></i></a></div>&nbsp;<a onClick=\"delRow(this)\" href=\"javascript:;\"><i class=\"fa fa-minus\"></i></a>&nbsp;<div class=\"btn-group\"><a onClick=\"getGrossProfit(this)\" href=\"javascript:void(0)\" data-toggle=\"modal\"><i class=\"fa fa-pencil\"></i></a></div></td></tr>";
		//通过表体id把显示文本显示到网页中
		$("#grossProfitData").append(html);
	}else if(addOrUpdate == "update"){
		tr.find("td:eq(0)").text($("#spec").find("option:selected").text()); 
		tr.find("td:eq(1)").text($("#productSize").val()) ; 
		tr.find("td:eq(2)").text($("#custGMarginRate").val()) ; 
		tr.find("td:eq(3)").text($("#prodEvalCostR").val()) ; 
		tr.find("td:eq(4)").text($("#prodEvalCostU").val()) ; 
		tr.find("td:eq(5)").text($("#quotePrice").val()) ; 
		tr.find("td:eq(6)").text($("#implementPrice").val()) ; 
		tr.find("td:eq(7)").text($("#implGMarginRate").val()) ; 
		tr.find("td:eq(8)").text($("#implGMarginCostU").val()) ; 
		tr.find("td:eq(9)").text($("#implPriceCut").val()) ; 
	}
}
...
function getGrossProfit(obj) {
	...
	$("#spec").val(specCur); 
	$("#custGMarginRate").val(tr.find("td:eq(2)").html());
//	$("#prodEvalCostR").val(tr.find("td:eq(2)").html());
//	$("#prodEvalCostU").val(tr.find("td:eq(3)").html());
//	$("#quotePrice").val(tr.find("td:eq(4)").html());
	$("#implementPrice").val(tr.find("td:eq(6)").html());
	$("#implGMarginRate").val(tr.find("td:eq(7)").html());
	$("#implGMarginCostU").val(tr.find("td:eq(8)").html());
	$("#implPriceCut").val(tr.find("td:eq(9)").html());
	if(typeof(window.parent.evalHeadData)!="undefined" && window.parent.evalHeadData.hasOwnProperty("grossProfitRate")){
		$("#custGMarginRate").val(window.parent.evalHeadData.grossProfitRate);
	}else{
		$("#custGMarginRate").val();
...
function getGrossProfitData(){
	...
	for(var i=1;i<valueTrs.length;i++){
		...
		grossProfitResult.push({"productName":tds[0].innerText,"productSize":tds[1].innerText,"expectedProfitRate":tds[2].innerText,
			"productCost":tds[3].innerText,"productEvalCost":tds[4].innerText,"quotePrice":tds[5].innerText,
			"implementPrice":tds[6].innerText,"imptProfitRate":tds[7].innerText,"imptProfitCost":tds[8].innerText,
			"implPriceCut":tds[9].innerText});
	}
...
function showResultData(){
	...
	for(var i=0;i<valueTrs.length;i++){
		...
		html = html + "<tr><td>"+tdtmp[0].innerText+"</td><td>"+tdtmp[1].innerText+"</td><td>"+tdtmp[2].innerText+"</td><td>"+tdtmp[4].innerText+"</td><td>"+tdtmp[5].innerText+"</td><td>"+tdtmp[6].innerText+"</td><td>"+tdtmp[7].innerText+"</td><td>"+tdtmp[9].innerText+"</td></tr>"
	}
...
function saveEvalDetail(){
	...
	var profitValue = $('#grossProfitTable').find("tr").eq(1).children("td").eq(4).text();
	if(profitValue==""){
		alert("数据未填充完整，请编辑！");
		return;
	}
...
function changeCurrencyType(){
	  if(evalHeadData.quoteCurrency == "USD"){
		  $("#grossProfitTable").find("tr")[0].children[4].textContent = "产品测算成本（$）";
		  $("#grossProfitTable").find("tr")[0].children[5].textContent = "测算报价（$）";
		  $("#grossProfitTable").find("tr")[0].children[6].textContent = "执行金额（$）";
...
function viewPage(jsonData){
	...
	for(var i=0;i<jsonData.length;i++){
		html += "<tr><td>" +jsonData[i].productName+ "</td><td>" +jsonData[i].productSize
		...
		+ "</td><td>" +jsonData[i].imptProfitRate+ "</td><td hidden=\"hidden\">" +jsonData[i].imptProfitCost+ "</td><td hidden=\"hidden\">" +jsonData[i].implPriceCut
		+ "</td><td><div class=\"btn-group\"><a onclick=\"addGrossProfit()\" href=\"javascript:void(0);\"><i class=\"fa fa-plus\"></i></a></div>&nbsp;<a onClick=\"delRow(this)\" href=\"javascript:;\"><i class=\"fa fa-minus\"></i></a>&nbsp;<div class=\"btn-group\"><a onClick=\"getGrossProfit(this)\" href=\"javascript:void(0)\" data-toggle=\"modal\"><i class=\"fa fa-pencil\"></i></a></div></td></tr>";
	
		html4ResultPage += "<tr><td>"+jsonData[i].productName+"</td><td>"+jsonData[i].productSize
		+"</td><td>"+jsonData[i].expectedProfitRate+"</td><td>"+jsonData[i].productEvalCost
		+"</td><td>"+jsonData[i].quotePrice+"</td><td>"+jsonData[i].implementPrice
