...
//生成工单
function createJOrder(headerId){
	var url = "/JHTEvaluation/getSoLineByHeaderId.do";
	$.post(url , {"headerId":headerId} , function(json){
		console.log(json);
		//var soLineList = $.parseJSON(json);
...
//       },//传递参数（*）
      ...
      // showToggle:false,     //是否显示详细视图和列表视图的切换按钮  	
       ...
       {title: '保存日期',field: 'orderDate', align: 'center',valign: 'middle'},  
       {title: '销售订单状态',field: 'so_Status',align: 'center',valign: 'middle'},  
       {title: '采购订单状态',field: 'po_Status',align: 'center'},
       {title: '工单状态',field: 'jo_Status', align: 'center',valign: 'middle'},  
       ...
       {title: '操作', field: 'id',align: 'center',formatter:function(value,index){  
       	console.log(index);
       	var d = '  <a title="生成生产加工合同" href="javaScript:createPurOrder();" class="active" ><i class="fa fa-copy text-active"></i></a>&nbsp;'
       	+'<a title="生成工单" href="javaScript:createJOrder(\''+$.trim(index.headerId)+'\');" class="active"><i class="fa fa-file-o"></i></a>&nbsp;'
       	+'<a title="回签" href="javaScript:signBack();" class="active"><i class="fa fa-reply"></i></a>&nbsp;'
       	+'<a title="添加用料单" href="javaScript:createMOrder();" class="active"><i class="fa fa-file"></i></a>&nbsp;';
...
function initJobTable(soLineList){  
	...
	$table.bootstrapTable({  
      ...
      {title: '成品名称',field: 'productSKU',align: 'center',valign: 'middle'},
      {title: '规格',field: 'specification',align: 'center',valign: 'middle'},  
      {title: '报价',field: 'unitPrice', align: 'center',valign: 'middle'},  
      {title: '操作', field: 'id',align: 'center',formatter:function(value,index){  
      	var d = ' <a title="工单预览" href="javaScript:void(0);" onClick="checkJobOrder(\''+index.evaluationOrderNumber+'\');" class="active"><i class="fa fa-file-o"></i></a>&nbsp;&nbsp;<a onclick="addProduct();" href="javaScript:void(0);"><i class="fa fa-plus"></i></a>';
      	return d;  
      } } 
      ]  
  });  
...
//工单预览
function checkJobOrder(evaluationOrderNumber){
	var url = "/JHTEvaluation/getJobOrdersByEvalationNumber.do";
	$.post(url , {"evaluationOrderNumber":evaluationOrderNumber} , function(json){
		console.log(json);
		var joList = $.parseJSON(json);

		for (var i = 0; i < joList.length; i++) {
			var joOrder = joList[i];
			var productName = joOrder.joHeader.materialName;
			var productSKU = joOrder.joHeader.materialSku;
			var productSpec = "";
			var productPrice = "";
