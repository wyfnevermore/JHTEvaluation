...
public class productDescriptionController {
	...
	public String componentUpdate(@RequestBody List<Component> compoList,String evalDetailID,String workDesc,String packageDesc,String evalRemark,String lightColorVal,String middleColorVal,String deepColorVal) {
		String result = "{'flag':false,'msg':''}";
		try{
			workDesc = new String(workDesc.getBytes("ISO-8859-1"),"UTF-8");
			packageDesc = new String(packageDesc.getBytes("ISO-8859-1"),"UTF-8");
			evalRemark = new String(evalRemark.getBytes("ISO-8859-1"),"UTF-8");
			
			EvalDetail evalDetailInit = new EvalDetail();
			evalDetailInit = evalDetailService.findById(evalDetailID);
			EvalDetail evalDetailNew = new EvalDetail();
			evalDetailNew.setWorkDesc(workDesc);
			evalDetailNew.setPackageDesc(packageDesc);
			evalDetailNew.setEvalRemark(evalRemark);
			evalDetailNew.setLightColorVal(Float.parseFloat(lightColorVal));
			evalDetailNew.setMiddleColorVal(Float.parseFloat(middleColorVal));
			evalDetailNew.setDeepColorVal(Float.parseFloat(deepColorVal));
			List<Component> compListInit = new ArrayList<Component>();
			compListInit = componentService.getCompListByDetailID(evalDetailID);
			
//			componentService.editEvalDetail(evalDetailInit,evalDetailNew);
//			componentService.editCompList(compListInit,compoList);
		}catch(Exception e){
			result = "{'flag':false,'msg':'"+e.getMessage()+"'}";
			e.printStackTrace();
		}
