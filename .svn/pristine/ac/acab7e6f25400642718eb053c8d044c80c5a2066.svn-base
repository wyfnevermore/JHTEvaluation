$(function(){
    var _upFile=document.getElementById("uploadFile");
    ...
    if (_upFile.files.length === 0) {  
        alert("请选择图片");  
        return; }  
    var oFile = _upFile.files[0];//获得一张图片的方法 
    ...
    if(!new RegExp("(jpg|jpeg|png)+","gi").test(oFile.type)){  
        ...
        return;  
            ...
            reader.onload = function(e) {
                ...
                //压缩前预览 
                var imgId = parseInt(Math.random()*1000);
  				photosOriginal.push({"src":base64Img,"id":imgId});
                //--执行resize。  
                var _ir=ImageResizer({  
                        resizeMode:"auto"  
                        ,dataSource:base64Img  
                        ,dataSourceType:"base64"  
                        ,maxWidth:425 //允许的最大宽度  
                        ...
                        ,success:function(resizeImgBase64,canvas){
                            //压缩后预览
                            updateImg(resizeImgBase64,imgId);
...
var innerTools={  
        getBase4FromImgFile:function(file,callBack){  
            ...
            reader.onload = function(e) {  
                var base64Img= e.target.result;  
                //var jQueryimg = jQuery('<img>').attr("src", e.target.result)  
                //jQuery('#preview').empty().append(jQueryimg)  
                if(callBack){  
                    callBack(base64Img);  
                }  
            };  
    ...
    //--处理数据源。。。。将所有数据源都处理成为图片图片对象，方便处理。  
        ,getImgFromDataSource:function(datasource,dataSourceType,callback){  
            ...
            img1.src=jQuery(datasource).attr("src");  
            if(callback){  
             callback(img1);  
            }  
            }  
    ...
    ,getResizeSizeFromImg:function(img){  
       var _img_info={  
                w:jQuery(img)[0].naturalWidth,  
                h:jQuery(img)[0].naturalHeight  
            };  
       var _resize_info={  
           w:0  
           ,h:0  
    ...
    (function(){  
        innerTools.getImgFromDataSource(settings.dataSource,settings.dataSourceType,function(_tmp_img){  
            ...
            setTimeout(function(){
                ...
                var _img_info={  
                    w:jQuery(__tmpImg)[0].naturalWidth,  
                    h:jQuery(__tmpImg)[0].naturalHeight  
                };  
