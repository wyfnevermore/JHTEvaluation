			...
			for(var i=1;i<4;i++){
				if($(colorInputTds[i]).attr("name")!="colorName" && $(colorInputTds[i]).attr("name")!="otherCheckBox"){
					if($(colorInputTds[i]).val()!=""){
						hasValFlag = true;
						if(($(compTr.children[0]).text()=="组件1")&&(colorCostArray[i-1]=="")){
							alert("请补充价格");
...
function setCompRatio(totalObj){
	var pieCompRateResult = {};
	var resultJson = {};
	var keyTmp = "";
	var valueTmp = 0;
	var compCountTotal = 0;
	$("#compBody").find("tr").each(function(){
		...
		if(resultJson.hasOwnProperty(keyTmp)){
			resultJson[keyTmp] = resultJson[keyTmp] + valueTmp;
			compCountTotal+=valueTmp;
		}else{
			resultJson[keyTmp] = valueTmp;
			compCountTotal+=valueTmp;
		}
	});
	$("#compBody").find("tr").each(function(){
		keyTmp = $(this).find("td:eq(4)").find("input").val();
		if(resultJson.hasOwnProperty(keyTmp)){
			$(this).find("td:eq(6)").find("input").val((100*resultJson[keyTmp]/compCountTotal).toFixed(2));
			pieCompRateResult[keyTmp] = resultJson[keyTmp];
		}
	});
	window.parent.setCompPieData2Mach(pieCompRateResult);
}
 ...
 * @returns
 */
function getCompJson2DB(){
	var jsonResult=[];
	var valueTrs = $("#compoSel").find("tr");
	var tds;
	for(var i=1;i<valueTrs.length;i++){
		tds = valueTrs[i].children;
		jsonResult.push({"compNo":tds[0].innerText,"compName":tds[4].children[0].value,"compRatio":tds[6].children[0].value,"compCount":tds[5].children[0].value,"patternJson":tds[1].children[0].value,"netTypeJson":tds[2].children[0].value,"specJson":tds[3].children[0].value,"specSizeOtherName":tds[11].children[0].value});
	}
	return jsonResult;
}
...
function getExcelCompJosnStr(){
	...
	for(var i=1;i<valueTrs.length;i++){
		tds = valueTrs[i].children;
		jsonResult.push({"compName":tds[4].children[0].value,"compCount":tds[5].children[0].value,"specJson":tds[3].children[0].value});
	}
...
function saveComponentList(){
	...
	$.ajax({
		...
		data : JSON.stringify(getCompJson2DB()),
		dataType : "text",
...
function updateComponentList(){
	...
	$.ajax({
		...
		async : false,
		contentType : 'application/json;charset=utf-8',
		url : "/JHTEvaluation/componentUpdate.do?evalDetailID="+evalDetailID+"&workDesc="+$("#workDesc").val()+"&packageDesc="+$("#packageDesc").val()+"&evalRemark="+$("#evalRemark").val()+"&lightColorVal="+eval_colorCost["浅"]+"&middleColorVal="+eval_colorCost["中"]+"&deepColorVal="+eval_colorCost["深"]+"&",
		data : JSON.stringify(getCompJson2DB()),
		dataType : "text",
		success : function(json) {
			var jsonObj = eval('('+json+')');
...
function showColorForm(obj){
	...
	$("#colorSelHead").find("input").each(function(){
		$(this).val("");
	});
	...
	var firstColorName = colorObj[0].firstColorName;
...
function viewPage(jsonData){
	...
	for(var i=0;i<jsonData.length;i++){
		if(jsonData[i].compNo=="组件1"){//材料里规格长宽高恢复的时候用
			var parentSpecJson = {};
			var curSpecJsonArray = eval('('+jsonData[i].specJson+')');
			for(var j=0;j<curSpecJsonArray.length;j++){
				parentSpecJson[curSpecJsonArray[j].specName] = {"Length":curSpecJsonArray[j].length,"Width":curSpecJsonArray[j].width,"Height":curSpecJsonArray[j].height,"other":curSpecJsonArray[j].other};
			}
			window.parent.specSel = parentSpecJson;
		}
		html += "<tr><td>"+jsonData[i].compNo+"</td>"
	      +"<td hidden=\"hidden\"><input type=\"hidden\" value='"+jsonData[i].patternJson+"'/></td>" 
	      +"<td hidden=\"hidden\"><input type=\"hidden\" value='"+jsonData[i].netTypeJson+"'/></td>"
	      +"<td hidden=\"hidden\"><input type=\"hidden\" value='"+jsonData[i].specJson+"'/></td>"
		  +"<td><input type=\"text\" class=\"form-control\" data-required=\"true\" value='"+jsonData[i].compName+"'></td>"
		  +"<td><input type=\"text\" class=\"form-control\" data-required=\"true\" data-type=\"number\" value='"+jsonData[i].compCount+"'></td>"
		  +"<td><input type=\"text\" class=\"form-control\" value='"+jsonData[i].compRatio+"'></td>"
	      +"<td><a onclick=\"showColorForm(this)\" href=\"javascript:void(0)\" class=\"btn btn-sm btn-icon btn-danger\"><i class=\"fa fa-plus\"></i></a></td>"
	      +"<td><a onclick=\"showPatternForm(this)\" href=\"javascript:void(0)\" class=\"btn btn-sm btn-icon btn-danger\" data-toggle=\"modal\"><i class=\"fa fa-plus\"></i></a></td>"
	      +"<td><a onclick=\"showSpecForm(this)\" href=\"javascript:void(0)\" class=\"btn btn-sm btn-icon btn-danger\" data-toggle=\"modal\"><i class=\"fa fa-plus\"></i></a></td>"
	      +"<td><div class=\"btn-group\"><a onclick=\"addCompRow()\" href=\"javascript:void(0)\" data-toggle=\"modal\"><i class=\"fa fa-plus\"></i></a></div>"
